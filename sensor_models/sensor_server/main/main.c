...
/* Add a function to publish current sensor data */
void example_ble_mesh_publish_sensor_data(void)
{
    uint8_t *status = NULL;
    uint16_t buf_size = 0;
    uint16_t length = 0;
    ...
    // --- Start: Simulate reading new sensor data and update buffers ---
    // Generate random values for temperature (e.g., 15 to 30 degrees Celsius)
    // and humidity (e.g., 40 to 80 percent).
    // Temperature 8 characteristic stores value * 2.
    temperature = (int8_t)(rand() % (60 - 30 + 1) + 30); // Range 30-60 for temperature*2 (15-30 C)
    humidity = (int8_t)(rand() % (80 - 40 + 1) + 40);   // Range 40-80 for humidity (%)

    // Reset the net_buf_simple buffers' write pointers
    net_buf_simple_reset(&sensor_data_0);
    net_buf_simple_reset(&sensor_data_1);

    // Add the new random values to the buffers
    net_buf_simple_add_u8(&sensor_data_0, (uint8_t)temperature);
    net_buf_simple_add_u8(&sensor_data_1, (uint8_t)humidity);
    // --- End: Simulate reading new sensor data and update buffers ---

    // Calculate the required buffer size, similar to example_ble_mesh_send_sensor_status
    for (i = 0; i < ARRAY_SIZE(sensor_states); i++) {
        esp_ble_mesh_sensor_state_t *state = &sensor_states[i];
        if (state->sensor_data.length == ESP_BLE_MESH_SENSOR_DATA_ZERO_LEN) {
            buf_size += ESP_BLE_MESH_SENSOR_DATA_FORMAT_B_MPID_LEN;
    ...
    // Populate the buffer with marshalled sensor data for all states
    for (i = 0; i < ARRAY_SIZE(sensor_states); i++) {
         length += example_ble_mesh_get_sensor_data(&sensor_states[i], status + length);
    }
...
{
    const TickType_t xDelay = pdMS_TO_TICKS(1000); // Delay for 1 second

    while (1) {
        // Call the publish function
        example_ble_mesh_publish_sensor_data();

        // Wait for 1 second
        vTaskDelay(xDelay);
    }
    // Tasks should not return, but if it does, delete it
    vTaskDelete(NULL);
}
...
void app_main(void)
{
    ...
    // Create the publish task after BLE Mesh initialization
    xTaskCreate(publish_task, "publish_sens", 2048, NULL, 5, NULL);
}
